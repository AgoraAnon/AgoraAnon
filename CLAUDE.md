# AgoraAnon - Claude Code Context

## Shared AI Coordination

Read `~/xuserbin/AINOTES.md` before using this file. `AINOTES.md` is the host-level source of truth and handoff surface for Claude Code and ChatGPT Codex.

If this file conflicts with `AINOTES.md` about the current host, workspace, shell, or shared workflow, follow `AINOTES.md`. After meaningful work, update `AINOTES.md` if there is durable system/project context or a handoff for the other AI.

## Project Overview

AgoraAnon is a donation landing page — a single place the owner can link people to when seeking support for their projects or causes. There is no source code, build system, or dependencies; the entire project is a single `README.md` file.

## Repository Purpose

- A shareable donation page that consolidates all cryptocurrency addresses in one place
- The owner directs supporters here so they can choose their preferred coin to donate
- Social contact: X/Twitter [@AgoraAnon](https://x.com/AgoraAnon)
- Accepts donations across 16+ addresses spanning privacy coins (XMR, DERO, WOW, ARRR, FIRO) and mainstream chains (BTC, ETH, LTC, BCH, BNB, MATIC, KMD)

## Working in This Repo

Since this is a docs-only repo, all changes are edits to `README.md`.

- No build, test, or lint commands
- No package manager or dependencies to install
- Changes are committed and pushed directly

## Development Branch

Active development branch: `claude/document-project-overview-WYRdJ`
